/* ═══════════════════════════════════════════════════════════
   ...
   Manzil/xarita YO'Q — buyurtma botga sendData orqali uzatiladi,
   lokatsiya va to'lov bot ichida so'raladi.
   ═══════════════════════════════════════════════════════════ */
'use strict';
...
const I18N = {
  ...
  finish_in_bot: { uz: 'Lokatsiya va to‘lov bot orqali so‘raladi', ru: 'Локацию и оплату запросит бот', en: 'The bot will ask for location & payment' },
  only_in_tg: { uz: 'Bu faqat Telegram ichida ishlaydi', ru: 'Доступно только в Telegram', en: 'Only works inside Telegram' },
...
/* ── Checkout (manzil YO'Q — faqat yetkazish turi + to'lov) ── */
function openCheckout() {
  el('checkoutContent').innerHTML = `
    ...
    <div class="field"><label>${L('payment')}</label>
      <div class="seg" id="segPay">
        <button class="active" data-v="cash"><span data-ic="cash"></span>${L('cash')}</button>
        <button data-v="card"><span data-ic="card"></span>${L('card')}</button>
      </div>
    </div>
    <div class="field"><label>${L('note')}</label><textarea id="ckNote" placeholder="${L('note')}"></textarea></div>
    <div id="ckSummary"></div>
    <p style="text-align:center;color:var(--muted);font-size:12.5px;margin:2px 0 14px">${L('finish_in_bot')}</p>
    <button class="btn" id="submitOrder"><span data-ic="bag"></span>${L('order_btn')}</button>`;
  let deliveryType = 'delivery', payment = 'cash';
  const segD = el('segDelivery'), segP = el('segPay');
  segD.querySelectorAll('button').forEach(b => b.onclick = () => { segD.querySelectorAll('button').forEach(x => x.classList.remove('active')); b.classList.add('active'); deliveryType = b.dataset.v; renderSummary(deliveryType); });
  ...
  el('submitOrder').onclick = () => submitOrder(deliveryType, payment, el('ckNote').value.trim());
...
function submitOrder(deliveryType, paymentMethod, note) {
  ...
  const payload = { v: 1, items: State.cart.map(x => ({ id: x.id, qty: x.qty })), delivery_type: deliveryType, payment_method: paymentMethod, note: note || '' };
  if (tg && typeof tg.sendData === 'function') {
    try {
      tg.sendData(JSON.stringify(payload));   // Mini App yopiladi va ma'lumot botga uzatiladi
      State.cart = []; saveCart();
      // (sendData odatda ilovani avtomatik yopadi)
...
function closeSheets() { document.querySelectorAll('.sheet').forEach(s => s.classList.remove('open')); if (tg && tg.BackButton) tg.BackButton.hide(); }
